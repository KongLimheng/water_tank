    ...
    <>
      ...
      <div className={`fixed top-0 right-0 h-full w-full sm:w-[400px] bg-white z-50 shadow-2xl transform transition-transform duration-300 ease-in-out flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="p-5 border-b border-slate-100 flex items-center justify-between bg-white">
          <h2 className="text-xl font-bold text-slate-900">Your Cart ({cart.reduce((a,c) => a + c.quantity, 0)})</h2>
          ...
          {cart.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center space-y-4">
                ...
                <div>
                    ...
                    <p className="text-slate-500 text-sm mt-1">Looks like you haven't added any hydration yet.</p>
                </div>
                <button 
                  onClick={onClose}
              ...
              <div key={item.id} className="flex gap-4 p-3 bg-slate-50 rounded-xl border border-slate-100">
                ...
                <div className="flex-1 flex flex-col justify-between">
                  <div>
                    ...
                    <p className="text-primary-600 font-bold text-sm">${item.price.toFixed(2)}</p>
                  </div>
